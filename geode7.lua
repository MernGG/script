...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if (not TeleportCheck) and queue_on_teleport then
		...
		queue_on_teleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/MernGG/script/refs/heads/master/geode7.lua'))()")
	end
end)
...
local function getRandomPublicServer()
	...
	if not response or not response.data or typeof(response.data) ~= "table" then
